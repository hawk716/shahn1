...
export async function POST(request: NextRequest) {
  try {
    ...
    const { sender_name, amount, app_name } = page

    // Search for unused matching payment
    const payment = await findUnusedPayment(sender_name, parseFloat(amount), app_name)

    if (!payment) {
      await logVerification({
        requested_name: sender_name,
        requested_amount: parseFloat(amount),
        requested_app: app_name,
        matched_payment_id: null,
        success: false,
        failure_reason: "No matching unused payment found (via payment page)",
    ...
    const creditedBalance = parseFloat(amount) * exchangeRate
